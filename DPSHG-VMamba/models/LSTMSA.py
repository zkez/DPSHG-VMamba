...
class GatedTransLSTMLayer(nn.Module):
    # Constructor
    def __init__(self, input_channels, hidden_channels,
                 kernel_size, bias, attenion_size):
        ...
        self.conv = nn.Conv3d(in_channels=6, out_channels=24, kernel_size=1, stride=1, padding=0)
...
class LSTMSA(nn.Module):
    def __init__(self, input_channels=64, hidden_channels=[64], kernel_size=5, bias=True, attenion_size=224*224):
        super(LSTMSA, self).__init__()
        self.lstmlayer = GatedTransLSTMLayer(input_channels, hidden_channels, kernel_size, bias, attenion_size)
...
if __name__ == "__main__":
    x1 = torch.randn(1,8,16,16,24).to("cuda")
    x2 = torch.randn(1,8,16,16,24).to("cuda")
    x3 = torch.randn(1,8,16,16,24).to("cuda")
    model = LSTMSA(input_channels=24, hidden_channels=[24], kernel_size=3, bias=True, attenion_size=96).to("cuda")
    output = model(x1, x2, x3)
    print(f"Output shape: {output.shape}")  # (1, 8, 16, 16, 24)